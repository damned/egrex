...
describe 'SpecifiedTokenExtractor' do
  ...
  it "should return tokens ordered by their names' location in example, regardless of specifiers ordering" do
    tokens, token_specifications = SpecifiedTokenExtractor.new.
        tokenize('FirstSecond', {
            'Second' => :a_specifier,
  ...
  it 'should include parts of example not matched by specifier names as nil-valued specifiers' do
    tokens, token_specifications = SpecifiedTokenExtractor.new.
        tokenize('FirstUnspecifiedLast',
                 'First' => :a_specifier,
